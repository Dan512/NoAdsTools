...
| ---------------------------------------------------- | ------- | -------------- | -------- | ------------------------------------- | --- |
| [JSZip](https://stuk.github.io/jszip/)               | 3.10.1  | MIT or GPL-3.0 | **MIT**  | `photo-editor/js/vendor/jszip.min.js`              | Batch export ZIP archive (Phase 10) |
| [jsPDF](https://github.com/parallax/jsPDF)           | 3.0.4   | MIT            | MIT      | `photo-editor/js/vendor/jspdf/jspdf.umd.min.js` (~419 KB) + `LICENSE` | Image-to-PDF export (v1.1 Feature 4) |
...
- **JSZip** is dual-licensed (MIT-or-GPL-3.0). We pick **MIT**, the more
  ...
  `photo-editor/js/vendor/jszip.min.js` header comment.
- **jsPDF** is **MIT**. Attribution preserved in the unmodified
  `photo-editor/js/vendor/jspdf/jspdf.umd.min.js` header comment + `LICENSE` in the same
...
- **Tesseract.js + tesseract.js-core** are both **Apache-2.0** by Naptha.
  License preserved at `photo-editor/js/vendor/tesseract/.notice`. We vendor LSTM-only
...
- `photo-editor/js/vendor/bgremove/` is ~118 MB (170 KB code + 34 KB license + 9 KB
  manifest + ~117 MB of chunked binary data for the CPU + WebGPU ORT
  kernels and the ISNET fp16 model).
- `photo-editor/js/vendor/onnxruntime-web/` is ~36 MB (CPU bundle + WebGPU bundle +
  4 threaded JSEP WASM kernel files + LICENSE).
- `photo-editor/js/vendor/jszip.min.js` is ~97 KB.
- `photo-editor/js/vendor/jspdf/` is ~420 KB (UMD bundle + LICENSE).
- `photo-editor/js/vendor/heic/` is ~1.1 MB (80 KB JS + 1.0 MB WASM + 43 KB
  LICENSE), and `vendor/libheif/` is a second, byte-identical ~1.1 MB copy
  serving the non-editor tools.
- `photo-editor/js/vendor/blazeface/` is ~600 KB (78 KB ONNX graph + 517 KB external
  weights + 3 KB metadata + `.notice`).
- `photo-editor/js/vendor/tesseract/` is ~22 MB (174 KB lib JS + 6 LSTM-only WASM
  variants + 1.9 MB gzipped English language data + `.notice`).
- `vendor/mediabunny/` is ~0.66 MB (660 KB minified bundle + ~17 KB LICENSE).
- The total `photo-editor/js/vendor/` footprint is ~155 MB, dominated by the chunked
