...
import { parseRanges, everyN, burst } from '../../js/ranges.js';
...
test('whitespace tolerant', () => {
  assert.deepEqual(parseRanges('  1 - 3 ,5 ', 10).groups, [[1,3],[5,5]]);
});
...
test('errors on reversed, zero, non-numeric, and fully-out-of-range tokens', () => {
  ...
  assert.deepEqual(mixed.groups, [[1,3],[5,5]]);
  assert.equal(mixed.errors.length, 1);
});
test('empty / whitespace-only input → no groups, no error', () => {
  assert.deepEqual(parseRanges('', 10), { groups: [], flat: [], errors: [] });
});
test('everyN chunks with a short final chunk', () => {
  assert.deepEqual(everyN(10, 3), [[1,3],[4,6],[7,9],[10,10]]);
  assert.deepEqual(everyN(10, 20), [[1,10]]);   // N >= count → whole doc
  assert.deepEqual(everyN(6, 2), [[1,2],[3,4],[5,6]]);
});
test('burst → one group per page', () => {
  assert.deepEqual(burst(3), [[1,1],[2,2],[3,3]]);
});
