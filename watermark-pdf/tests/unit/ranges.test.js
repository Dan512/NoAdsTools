...
test('parses mixed single + range tokens (1-based), builds a deduped flat list', () => {
...
test('dedups the flat page list', () => {
  assert.deepEqual(parseRanges('1,1,2', 10).flat, [1,2]);
});
test('clamps a partially-out-of-range token to the page count', () => {
  assert.deepEqual(parseRanges('8-15', 10).flat, [8,9,10]);
});
...
test('empty / whitespace-only input → empty flat, no error', () => {
  const r = parseRanges('', 10);
  assert.deepEqual(r.flat, []);
  assert.deepEqual(r.errors, []);
});
